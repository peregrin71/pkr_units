...
TEST_F(SiAngleTest, radian_has_angle_dimension) {
    radian_t angle(1.0);
    static_assert(std::is_same_v<decltype(angle)::dimension_type, decltype(angle_dimension)>,
                  "radian_t should have angle_dimension");
}
...
TEST_F(SiAngleTest, scalar_multiply_radian_preserves_type) {
    radian_t angle(3.14159);
    auto result = angle * 2.0;
    static_assert(std::is_same_v<decltype(result), radian_t>,
                  "Scalar multiplication should preserve radian_t type");
    EXPECT_DOUBLE_EQ(result.value(), 6.28318);
}
...
TEST_F(SiAngleTest, scalar_multiply_degree_preserves_type) {
    degree_t angle(180.0);
    auto result = angle * 2.0;
    static_assert(std::is_same_v<decltype(result), degree_t>,
                  "Scalar multiplication should preserve degree_t type");
    EXPECT_DOUBLE_EQ(result.value(), 360.0);
}
...
TEST_F(SiAngleTest, scalar_multiply_gradian_preserves_type) {
    gradian_t angle(200.0);
    auto result = angle * 2.0;
    static_assert(std::is_same_v<decltype(result), gradian_t>,
