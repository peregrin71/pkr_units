...
TEST_F(SiCurrentOperatorsTest, add_amperes)
{
    ...
    auto result = a1 + a2;
    static_assert(std::is_same_v<decltype(result), pkr::units::ampere_t<double>>);
...
TEST_F(SiCurrentOperatorsTest, add_milliampere_to_ampere)
{
    ...
    static_assert(std::is_same_v<decltype(result), pkr::units::milliampere_t<double>>, "operator+ should preserve LHS type (milliampere)");
...
{
    pkr::units::ampere_t<double> a{10.0};
    auto result = a / 2.0;
...
{
    pkr::units::ampere_t<double> a1{5.0};
    pkr::units::ampere_t<double> a2{5.0};
    ASSERT_TRUE(a1 == a2);
}
...
{
    constexpr pkr::units::ampere_t<double> a{10.0};
