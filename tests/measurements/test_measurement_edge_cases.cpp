...
// Tests edge cases in measurement_rss_t.h that are not covered by existing tests
...
{
    pkr::units::measurement_rss_t<pkr::units::meter_t<double>> m{5.0, 0.0};
...
{
    pkr::units::measurement_rss_t<pkr::units::meter_t<double>> m{1.0, 1e-15};
...
{
    pkr::units::measurement_rss_t<pkr::units::meter_t<double>> m{1.0, 1000.0};
    EXPECT_DOUBLE_EQ(m.value(), 1.0);
...
{
    // Very large values
    pkr::units::measurement_rss_t<pkr::units::meter_t<double>> large{1e308, 1e300};
    EXPECT_DOUBLE_EQ(large.value(), 1e308);
    EXPECT_DOUBLE_EQ(large.uncertainty(), 1e300);

    // Very small values
    pkr::units::measurement_rss_t<pkr::units::meter_t<double>> small{1e-308, 1e-310};
...
TEST_F(MeasurementEdgeCasesTest, SpecialValues)
{
    ...
    // NaN value
    pkr::units::measurement_rss_t<pkr::units::meter_t<double>> nan_val{numeric_limits<double>::quiet_NaN(), 1.0};
    EXPECT_TRUE(std::isnan(nan_val.value()));
    EXPECT_DOUBLE_EQ(nan_val.uncertainty(), 1.0);

    // Infinity value
    pkr::units::measurement_rss_t<pkr::units::meter_t<double>> inf_val{numeric_limits<double>::infinity(), 1.0};
    EXPECT_TRUE(std::isinf(inf_val.value()));
    EXPECT_DOUBLE_EQ(inf_val.uncertainty(), 1.0);

    // NaN uncertainty
    pkr::units::measurement_rss_t<pkr::units::meter_t<double>> nan_unc{1.0, numeric_limits<double>::quiet_NaN()};
    EXPECT_DOUBLE_EQ(nan_unc.value(), 1.0);
    EXPECT_TRUE(std::isnan(nan_unc.uncertainty()));

    // Infinity uncertainty
    pkr::units::measurement_rss_t<pkr::units::meter_t<double>> inf_unc{1.0, numeric_limits<double>::infinity()};
...
{
    pkr::units::measurement_rss_t<pkr::units::meter_t<double>> zero_unc{5.0, 0.0};
    pkr::units::measurement_rss_t<pkr::units::meter_t<double>> normal{3.0, 0.1};
...
{
    pkr::units::measurement_rss_t<pkr::units::meter_t<double>> m1{5.0, 0.0};
    pkr::units::measurement_rss_t<pkr::units::meter_t<double>> m2{5.0, 0.1};
    pkr::units::measurement_rss_t<pkr::units::meter_t<double>> m3{5.1, 0.0};
...
TEST_F(MeasurementEdgeCasesTest, FormattingEdgeCases)
{
    ...
    pkr::units::measurement_rss_t<pkr::units::meter_t<double>> small_unc{1.0, 1e-10};
    std::string formatted = std::format("{}", small_unc);
    EXPECT_FALSE(formatted.empty());

    // Test formatting with very large uncertainty
    pkr::units::measurement_rss_t<pkr::units::meter_t<double>> large_unc{1.0, 1000.0};
...
{
    // Copy construction
    pkr::units::measurement_rss_t<pkr::units::meter_t<double>> original{10.0, 0.5};
    pkr::units::measurement_rss_t<pkr::units::meter_t<double>> copy = original;
