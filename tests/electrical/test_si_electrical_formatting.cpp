...
TEST_F(SiElectricalFormattingTest, volt_wide_formatting)
{
    ...
    std::wstring expected = std::wstring(L"12.5 ") + std::wstring(pkr::units::volt_t::w_symbol);
    ASSERT_EQ(result, expected);
}

TEST_F(SiElectricalFormattingTest, microvolt_wide_formatting)
{
    pkr::units::microvolt_t uv{2.5};
    std::wstring result = std::format(L"{}", uv);
    std::wstring expected = std::wstring(L"2.5 ") + std::wstring(pkr::units::microvolt_t::w_symbol);
    ASSERT_EQ(result, expected);
}
...
{
    pkr::units::millicoulomb_t mc{1.5};
...
{
    pkr::units::microcoulomb_t uc{7.0};
...
{
    pkr::units::microfarad_t uf{22.0};
...
{
    pkr::units::microsiemens_t us{12.0};
...
{
    pkr::units::microhenry_t uh{15.0};
