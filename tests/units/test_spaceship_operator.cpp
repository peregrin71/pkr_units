...
TEST_F(SpaceshipOperatorTest, RegularUnitsComparison)
{
    ...
    pkr::units::meter_t m3{3.0};
    pkr::units::kilometer_t km{0.005}; // 5 meters
...
TEST_F(SpaceshipOperatorTest, TemperatureUnitsComparison)
{
    ...
    pkr::units::celsius_t c2{10.0}; // 10°C = 283.15K = 50°F
    pkr::units::kelvin_t k2{283.15};
    pkr::units::fahrenheit_t f2{50.0};

    EXPECT_EQ(c2 <=> k2, std::strong_ordering::equal);
    EXPECT_EQ(c2 <=> f2, std::strong_ordering::equal);

    // Test ordering
    pkr::units::celsius_t c3{-10.0}; // colder than c1
