...
{
    pkr::units::kilojoule_t energy{5.0};
...
{
    pkr::units::millijoule_t energy{500000.0};
    ASSERT_DOUBLE_EQ(energy.value(), 500000.0);
}

TEST_F(EnergyTest, microjoule_construction)
{
    pkr::units::microjoule_t energy{1000000000.0};
...
{
    pkr::units::joule_t energy1{1000.0};
    pkr::units::joule_t energy2{energy1};
...
{
    pkr::units::joule_t energy1{1000.0};
    pkr::units::joule_t energy2{std::move(energy1)};
    ASSERT_DOUBLE_EQ(energy2.value(), 1000.0);
}
...
{
    pkr::units::joule_t energy{1.0};
    using dimension = typename std::decay_t<decltype(energy)>::dimension_type;
    static_assert(std::is_same_v<dimension::value_type, pkr::units::dimension_t>);
