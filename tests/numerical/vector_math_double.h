...
double compute_total_energy_double(const three_body_state_double_t& state)
{
    ...
    double r12 = std::sqrt(
        (state.pos1.x - state.pos2.x) * (state.pos1.x - state.pos2.x) + (state.pos1.y - state.pos2.y) * (state.pos1.y - state.pos2.y) +
        (state.pos1.z - state.pos2.z) * (state.pos1.z - state.pos2.z));
    double r13 = std::sqrt(
        (state.pos1.x - state.pos3.x) * (state.pos1.x - state.pos3.x) + (state.pos1.y - state.pos3.y) * (state.pos1.y - state.pos3.y) +
        (state.pos1.z - state.pos3.z) * (state.pos1.z - state.pos3.z));
    double r23 = std::sqrt(
        (state.pos2.x - state.pos3.x) * (state.pos2.x - state.pos3.x) + (state.pos2.y - state.pos3.y) * (state.pos2.y - state.pos3.y) +
