    ...
    for (int i = 0; i < steps; ++i) {
        ...
        if (diag_step >= 0 && i >= std::max(0, diag_step - 3) && i <= diag_step + 3) {
            ...
            three_body_state_si_t state2{
                state.pos1 + (half_dt * k1.dpos1), state.pos2 + (half_dt * k1.dpos2), state.pos3 + (half_dt * k1.dpos3),
                state.vel1 + (half_dt * k1.dvel1), state.vel2 + (half_dt * k1.dvel2), state.vel3 + (half_dt * k1.dvel3)
            };
            auto k2 = derivative_si(state2);
            three_body_state_si_t state3{
                state.pos1 + (half_dt * k2.dpos1), state.pos2 + (half_dt * k2.dpos2), state.pos3 + (half_dt * k2.dpos3),
                state.vel1 + (half_dt * k2.dvel1), state.vel2 + (half_dt * k2.dvel2), state.vel3 + (half_dt * k2.dvel3)
            };
            auto k3 = derivative_si(state3);
            three_body_state_si_t state4{
                state.pos1 + (dt_seconds * k3.dpos1), state.pos2 + (dt_seconds * k3.dpos2), state.pos3 + (dt_seconds * k3.dpos3),
                state.vel1 + (dt_seconds * k3.dvel1), state.vel2 + (dt_seconds * k3.dvel2), state.vel3 + (dt_seconds * k3.dvel3)
            };
            ...
            auto print_si_stage = [&](const char* name, const three_body_derivative_si_t &K) {
                auto mag1 = std::sqrt((K.dvel1.x.value()*K.dvel1.x.value()) + (K.dvel1.y.value()*K.dvel1.y.value()) + (K.dvel1.z.value()*K.dvel1.z.value()));
                std::cout << name << " SI dvel1 (m/s^2) = (" << K.dvel1.x.value() << ", " << K.dvel1.y.value() << ", " << K.dvel1.z.value() << ")";
                std::cout << "  (AU/yr^2) = (" << (K.dvel1.x / accel_conv).value() << ", " << (K.dvel1.y / accel_conv).value() << ", " << (K.dvel1.z / accel_conv).value() << ")";
                std::cout << "  |dvel1|=" << mag1 << "\n";
            };
            ...
            three_body_state_double_t pd_state = { {state.pos1.x.value(), state.pos1.y.value(), state.pos1.z.value(), state.pos1.w.value()},
            ...
            three_body_state_double_t pd_state2{
                {pd_state.pos1.x + half_dt_pd * k1_pd_state.pos1.x, pd_state.pos1.y + half_dt_pd * k1_pd_state.pos1.y, pd_state.pos1.z + half_dt_pd * k1_pd_state.pos1.z, 0.0},
                {pd_state.pos2.x + half_dt_pd * k1_pd_state.pos2.x, pd_state.pos2.y + half_dt_pd * k1_pd_state.pos2.y, pd_state.pos2.z + half_dt_pd * k1_pd_state.pos2.z, 0.0},
                {pd_state.pos3.x + half_dt_pd * k1_pd_state.pos3.x, pd_state.pos3.y + half_dt_pd * k1_pd_state.pos3.y, pd_state.pos3.z + half_dt_pd * k1_pd_state.pos3.z, 0.0},
                {pd_state.vel1.x + half_dt_pd * k1_pd_state.vel1.x, pd_state.vel1.y + half_dt_pd * k1_pd_state.vel1.y, pd_state.vel1.z + half_dt_pd * k1_pd_state.vel1.z, 0.0},
                {pd_state.vel2.x + half_dt_pd * k1_pd_state.vel2.x, pd_state.vel2.y + half_dt_pd * k1_pd_state.vel2.y, pd_state.vel2.z + half_dt_pd * k1_pd_state.vel2.z, 0.0},
                {pd_state.vel3.x + half_dt_pd * k1_pd_state.vel3.x, pd_state.vel3.y + half_dt_pd * k1_pd_state.vel3.y, pd_state.vel3.z + half_dt_pd * k1_pd_state.vel3.z, 0.0}
            };
        ...
        if (energy_drift > result.max_energy_drift) {
            ...
            auto v1_sq = pkr::units::math::dot(state.vel1, state.vel1);
            auto v2_sq = pkr::units::math::dot(state.vel2, state.vel2);
            auto v3_sq = pkr::units::math::dot(state.vel3, state.vel3);
            auto ke1 = pkr::units::math::stable_multiply(pkr::units::math::stable_multiply(0.5, m1_si), v1_sq);
            auto ke2 = pkr::units::math::stable_multiply(pkr::units::math::stable_multiply(0.5, m2_si), v2_sq);
            auto ke3 = pkr::units::math::stable_multiply(pkr::units::math::stable_multiply(0.5, m3_si), v3_sq);
            ...
            auto r12 = pkr::units::math::sqrt(pkr::units::math::dot(d12, d12));
            auto d13 = state.pos1 - state.pos3;
            auto r13 = pkr::units::math::sqrt(pkr::units::math::dot(d13, d13));
            auto d23 = state.pos2 - state.pos3;
            auto r23 = pkr::units::math::sqrt(pkr::units::math::dot(d23, d23));
            ...
            auto gm_sum = pkr::units::math::stable_multiply(G_si, pkr::units::math::stable_add(pkr::units::math::stable_add(pair12, pair13), pair23));
            result.max_drift_gm_sum = pkr::units::math::stable_multiply(-1.0, gm_sum).in_base_si_units().value();
        }
    }
...
three_body_simulation_result_t run_three_body_simulation_si_from_state(const three_body_state_si_t& init_state, int steps = 1000, double dt_years = 0.01, int diag_step = -1) {
    three_body_simulation_result_t result;
    result.steps_run = steps;
    result.time_step = dt_years;
    result.total_time = steps * dt_years;
    ...
    for (int i = 0; i < steps; ++i) {
        ...
        if (diag_step >= 0 && i >= std::max(0, diag_step - 3) && i <= diag_step + 3) {
            ...
            auto k3 = derivative_si(state3);
            three_body_state_si_t state4{
                state.pos1 + (dt * k3.dpos1), state.pos2 + (dt * k3.dpos2), state.pos3 + (dt * k3.dpos3),
                state.vel1 + (dt * k3.dvel1), state.vel2 + (dt * k3.dvel2), state.vel3 + (dt * k3.dvel3)
            };
            auto k4 = derivative_si(state4);
