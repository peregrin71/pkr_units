...
// math namespace removed - use pkr::units:: directly
...
pkr::units::measurement_t<pkr::units::joule_t> compute_total_energy_meas(const three_body_state_measurement_t& state)
{
    ...
    auto v1_squared = pkr::units::sum_of_squares_rss(state.vel1.x, state.vel1.y, state.vel1.z);
    auto v2_squared = pkr::units::sum_of_squares_rss(state.vel2.x, state.vel2.y, state.vel2.z);
    auto v3_squared = pkr::units::sum_of_squares_rss(state.vel3.x, state.vel3.y, state.vel3.z);
    ...
    auto half_m1 = pkr::units::multiply_rss(half_scalar, m1_meas);
    auto ke1 = pkr::units::multiply_rss(half_m1, v1_squared);

    auto half_m2 = pkr::units::multiply_rss(half_scalar, m2_meas);
    auto ke2 = pkr::units::multiply_rss(half_m2, v2_squared);

    auto half_m3 = pkr::units::multiply_rss(half_scalar, m3_meas);
    auto ke3 = pkr::units::multiply_rss(half_m3, v3_squared);

    auto kin12 = pkr::units::add_rss(ke1, ke2);
    auto kinetic = pkr::units::add_rss(kin12, ke3);

    // Potential energy: -G * m1*m2/r12 - G*m1*m3/r13 - G*m2*m3/r23
    auto diff12 = state.pos1 - state.pos2;
    auto r12_squared = pkr::units::add_rss(
        pkr::units::add_rss(pkr::units::multiply_rss(diff12.x, diff12.x), pkr::units::multiply_rss(diff12.y, diff12.y)),
        pkr::units::multiply_rss(diff12.z, diff12.z));
    auto r12 = pkr::units::sqrt_rss(r12_squared);

    auto diff13 = state.pos1 - state.pos3;
    auto r13_squared = pkr::units::add_rss(
        pkr::units::add_rss(pkr::units::multiply_rss(diff13.x, diff13.x), pkr::units::multiply_rss(diff13.y, diff13.y)),
        pkr::units::multiply_rss(diff13.z, diff13.z));
    auto r13 = pkr::units::sqrt_rss(r13_squared);

    auto diff23 = state.pos2 - state.pos3;
    auto r23_squared = pkr::units::add_rss(
        pkr::units::add_rss(pkr::units::multiply_rss(diff23.x, diff23.x), pkr::units::multiply_rss(diff23.y, diff23.y)),
        pkr::units::multiply_rss(diff23.z, diff23.z));
    auto r23 = pkr::units::sqrt_rss(r23_squared);

    auto potential = pkr::units::multiply_rss(
        -G_meas,
        pkr::units::add_rss(
            pkr::units::add_rss(
                pkr::units::divide_rss(pkr::units::multiply_rss(m1_meas, m2_meas), r12),
                pkr::units::divide_rss(pkr::units::multiply_rss(m1_meas, m3_meas), r13)),
            pkr::units::divide_rss(pkr::units::multiply_rss(m2_meas, m3_meas), r23)));

    return pkr::units::add_rss(kinetic, potential);
}
...
TEST(RK4MeasurementsTest, MultiplyRSSUsesScalarMeasurement)
{
    ...
    auto res = pkr::units::multiply_rss(half, m1_meas);
