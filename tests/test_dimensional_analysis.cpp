...
TEST_F(DimensionalAnalysisTest, pressure_times_time_squared)
{
    ...
    static_assert(std::is_same_v<decltype(energy_density.normalized()), pkr::units::details::unit_t<double, std::ratio<1, 1>, pkr::units::dimension_t{.length = -1, .mass = 1, .time = 0, .current = 0, .temperature = 0, .amount = 0, .intensity = 0, .angle = 0}>>, "Resulting type should be kg/m");
