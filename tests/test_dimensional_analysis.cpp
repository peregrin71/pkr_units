...
{
    // km/h * h = km
    pkr::units::kilometer_per_hour_t<double> speed{100.0}; // 100 km/h
    pkr::units::hour_t<double> time{2.0};                  // 2 hours
...
{
    // m/s * s = m
    pkr::units::meter_per_second_t<double> speed{10.0}; // 10 m/s
...
TEST_F(DimensionalAnalysisTest, acceleration_times_time_equals_velocity)
{
    ...
    pkr::units::second_t<double> time{2.0};                    // 2 seconds
...
{
    // kg * m/s² = N (Newton)
    pkr::units::kilogram_t<double> mass{5.0};                  // 5 kg
    pkr::units::meter_per_second_squared_t<double> accel{9.8}; // 9.8 m/s²
...
{
    // N * m = J (Joule)
    pkr::units::newton_t<double> force{10.0};  // 10 N
    pkr::units::meter_t<double> distance{5.0}; // 5 m
...
{
    // J / s = W (Watt)
    pkr::units::joule_t<double> energy{100.0}; // 100 J
    pkr::units::second_t<double> time{10.0};   // 10 s
...
{
    // W * s = J (Joule)
    pkr::units::watt_t<double> power{50.0};  // 50 W
    pkr::units::second_t<double> time{20.0}; // 20 s
