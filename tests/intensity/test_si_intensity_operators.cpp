...
TEST_F(SiIntensityOperatorsTest, add_candelas)
{
    ...
    auto result = cd1 + cd2;
    static_assert(std::is_same_v<decltype(result), pkr::units::candela_t<double>>);
...
TEST_F(SiIntensityOperatorsTest, add_millicandela_to_candela)
{
    ...
    static_assert(std::is_same_v<decltype(result), pkr::units::millicandela_t<double>>, "operator+ should preserve LHS type (millicandela)");
...
{
    pkr::units::candela_t<double> cd{10.0};
    auto result = cd / 2.0;
    ASSERT_DOUBLE_EQ(result.value(), 5.0);
}
...
{
    pkr::units::candela_t<double> cd1{5.0};
    pkr::units::candela_t<double> cd2{5.0};
    ASSERT_TRUE(cd1 == cd2);
}
...
{
    constexpr pkr::units::candela_t<double> cd{10.0};
