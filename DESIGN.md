...
**Decision**: Use template specialization with `derived_unit_type_t<type_t, ratio_t, dimension_t>` for mapping unit components to their strong type classes.

**Implementation**:

The `derived_unit_type_t` trait maps a triplet of (type, ratio, dimension) to the most specific strong type available:
...
1. **Extensibility**: Adding new unit types only requires adding new specializations—no changes to arithmetic operators or existing logic. Want to add `mile_t`? Just add:
   ```cpp
   template<>
   struct derived_unit_type_t<double, std::ratio<1609344, 1000>, length_dimension>
   {
       using type = mile_t;
   };
   ```
...
3. **Future String Formatting**: When implementing `std::formatter`, we can use `derived_unit_type_t` to get the canonical name:
   ```cpp
   template<typename T>
   struct std::formatter<T> {
       auto format(const T& unit, auto& ctx) {
           using derived = derived_unit_type_t<typename T::value_type, 
...
**Decision**: Implement `operator+`, `operator-`, `operator*`, and `operator/` as free functions (not class methods) that work with any `is_base_pkr_unit_c`.
...
Add new specializations to `derived_unit_type_t`:
```cpp
template<>
