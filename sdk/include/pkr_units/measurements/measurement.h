...
public:
    ...
    constexpr measurement_t(UnitT value, UnitT uncertainty)
        : m_value(value), m_uncertainty(uncertainty) {
    ...
    constexpr measurement_t(typename UnitT::value_type value, typename UnitT::value_type uncertainty)
        : m_value(UnitT{value}), m_uncertainty(UnitT{uncertainty}) {
        // Ensure uncertainty is non-negative
        if (uncertainty < 0) {
            m_uncertainty = UnitT{0};
        }
    ...
    constexpr auto operator*(const measurement_t<OtherUnitT>& other) const {
        ...
        using result_type = decltype(result_value);
        auto rel_uncertainty1 = m_uncertainty.value() / m_value.value();  // dimensionless
        auto rel_uncertainty2 = other.unit_uncertainty().value() / other.unit_value().value();  // dimensionless
    ...
    constexpr auto value() const { return m_value.value(); }

    // Get the uncertainty as raw double (convenience)
    constexpr auto uncertainty() const { return m_uncertainty.value(); }

    // Get the measured value with units
    constexpr const UnitT& unit_value() const { return m_value; }

    // Get the uncertainty with units
    constexpr const UnitT& unit_uncertainty() const { return m_uncertainty; }
