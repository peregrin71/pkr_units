...
// ============================================================================
// Math functions for measurement_t types
// ============================================================================
// Provides mathematical operations that work with measurement_t types,
// automatically propagating uncertainties through dimensional analysis.
...
constexpr auto add(const measurement_t<UnitT1>& a, const measurement_t<UnitT2>& b) {
    return a + b;
}

// Subtraction with uncertainty propagation
template<typename UnitT1, typename UnitT2>
constexpr auto subtract(const measurement_t<UnitT1>& a, const measurement_t<UnitT2>& b) {
    return a - b;
}

// Multiplication with uncertainty propagation
template<typename UnitT1, typename UnitT2>
constexpr auto multiply(const measurement_t<UnitT1>& a, const measurement_t<UnitT2>& b) {
    return a * b;
}

// Division with uncertainty propagation
template<typename UnitT1, typename UnitT2>
constexpr auto divide(const measurement_t<UnitT1>& a, const measurement_t<UnitT2>& b) {
    return a / b;
}
...
template<typename UnitT>
constexpr auto sqrt(const measurement_t<UnitT>& measurement) {
...
template<typename UnitT>
constexpr auto exp(const measurement_t<UnitT>& measurement) {
...
template<typename UnitT>
constexpr auto log(const measurement_t<UnitT>& measurement) {
...
constexpr auto pow(const measurement_t<UnitT>& base, const measurement_t<ExpUnitT>& exponent) {
...
template<typename UnitT>
constexpr auto pow(const measurement_t<UnitT>& base, double exponent) {
    // Use math::pow for the value (create dimensionless exponent)
    auto dimensionless_exp = details::unit_t<double, std::ratio<1>, scalar_dimension>{exponent};
...
template<typename UnitT>
    requires is_angle_unit_c<UnitT>
constexpr auto sin(const measurement_t<UnitT>& measurement) {
    // Use std::sin for the value (angle input in radians)
    auto result_value = details::unit_t<double, std::ratio<1>, scalar_dimension>{std::sin(measurement.value())};
...
template<typename UnitT>
    requires is_angle_unit_c<UnitT>
constexpr auto cos(const measurement_t<UnitT>& measurement) {
    // Use std::cos for the value (angle input in radians)
    auto result_value = details::unit_t<double, std::ratio<1>, scalar_dimension>{std::cos(measurement.value())};
...
template<typename UnitT>
    requires is_angle_unit_c<UnitT>
constexpr auto tan(const measurement_t<UnitT>& measurement) {
    // Use std::tan for the value (angle input in radians)
    auto result_value = details::unit_t<double, std::ratio<1>, scalar_dimension>{std::tan(measurement.value())};
...
template<typename UnitT>
constexpr auto combined_uncertainty(const measurement_t<UnitT>& measurement) {
    return measurement.uncertainty();
}

// Get relative uncertainty as a percentage
template<typename UnitT>
constexpr double relative_uncertainty_percent(const measurement_t<UnitT>& measurement) {
    return measurement.relative_uncertainty().value() * 100.0;
}

// Normalize a measurement to its canonical SI form
template<typename UnitT>
constexpr auto normalize(const measurement_t<UnitT>& measurement) {
    ...
    return measurement_t<result_type>{normalized_value, normalized_uncertainty};
}
