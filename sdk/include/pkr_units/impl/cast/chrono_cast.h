...
namespace details
{
    ...
    struct named_unit_type_t<type_t, ratio_t, time_dimension>
    {
...
template<typename target_unit_t, typename Rep, typename Period>
    requires std::is_base_of_v<typename target_unit_t::_base, target_unit_t> &&
             (details::is_pkr_unit<target_unit_t>::value_dimension == time_dimension)
constexpr target_unit_t unit_cast(std::chrono::duration<Rep, Period> d) noexcept
{
    ...
    constexpr target_type_t conversion_factor = details::compute_conversion_factor<target_type_t>(
        ...
        target_ratio::num, target_ratio::den);  // Convert chrono period to target unit ratio
...
template<typename Duration, typename source_unit_t>
    requires std::is_base_of_v<typename source_unit_t::_base, source_unit_t> &&
             (details::is_pkr_unit<source_unit_t>::value_dimension == time_dimension)
constexpr Duration unit_cast(const source_unit_t& source) noexcept
{
    ...
    constexpr source_type_t conversion_factor = details::compute_conversion_factor<source_type_t>(
        ...
        Duration::period::num, Duration::period::den);  // Convert unit ratio to Duration period
