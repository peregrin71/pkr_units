...
struct matrix_3d_t {
public:
    constexpr matrix_3d_t() : m_data{} {}
    
    constexpr matrix_3d_t(const std::array<std::array<T, 3>, 3>& init) : m_data{init} {}
    
    // Access operators
    constexpr std::array<T, 3>& operator[](size_t i) { return m_data[i]; }
    constexpr const std::array<T, 3>& operator[](size_t i) const { return m_data[i]; }
