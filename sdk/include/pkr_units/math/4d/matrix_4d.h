...
struct matrix_4d_t {
public:
    constexpr matrix_4d_t() : m_data{} {}
    
    constexpr matrix_4d_t(const std::array<std::array<T, 4>, 4>& init) : m_data{init} {}
    
    // Access operators
    constexpr std::array<T, 4>& operator[](size_t i) { return m_data[i]; }
    constexpr const std::array<T, 4>& operator[](size_t i) const { return m_data[i]; }
