...
struct vec_4d_measurements_rss_t
{
    ...
    vec_4d_measurements_rss_t(T x, T y, T z, T w = T{1.0, 0.0})
        : x{x}
        , y{y}
        , z{z}
        , w{w}
    ...
    {
        return vec_4d_measurements_rss_t{-x, -y, -z, -w};
    }
    ...
    // Calculate the magnitude (length) of the 4D measurement vector
    T magnitude() const noexcept
    {
        // For measurements, compute: sqrt(x^2 + y^2 + z^2 + w^2)
        // preserving the RSS uncertainty propagation
        auto sum_of_squares = (x * x + y * y) + (z * z + w * w);
        auto value_part = std::sqrt(sum_of_squares.value());
        auto uncertainty_part = sum_of_squares.uncertainty() / (2.0 * std::sqrt(sum_of_squares.value()));
        return T{value_part, uncertainty_part};
    }
};
...
constexpr vec_4d_measurements_rss_t<T> operator+(const vec_4d_measurements_rss_t<T>& a, const vec_4d_measurements_rss_t<T>& b) noexcept
{
    return vec_4d_measurements_rss_t<T>{
        pkr::units::add_rss(a.x, b.x), pkr::units::add_rss(a.y, b.y), pkr::units::add_rss(a.z, b.z), pkr::units::add_rss(a.w, b.w)};
}
...
{
    return vec_4d_measurements_rss_t<T>{
        pkr::units::subtract_rss(a.x, b.x), pkr::units::subtract_rss(a.y, b.y), pkr::units::subtract_rss(a.z, b.z), pkr::units::subtract_rss(a.w, b.w)};
}
...
constexpr vec_4d_measurements_rss_t<decltype(pkr::units::multiply_rss(std::declval<ScalarT>(), std::declval<VecT>()))>
    operator*(const ScalarT& scalar, const vec_4d_measurements_rss_t<VecT>& v)
{
...
constexpr vec_4d_measurements_rss_t<decltype(pkr::units::multiply_rss(std::declval<VecT>(), std::declval<ScalarT>()))>
    operator*(const vec_4d_measurements_rss_t<VecT>& v, const ScalarT& scalar)
{
...
{
    return vec_4d_measurements_rss_t<T>{
        pkr::units::divide_rss(v.x, scalar), pkr::units::divide_rss(v.y, scalar), pkr::units::divide_rss(v.z, scalar), pkr::units::divide_rss(v.w, scalar)};
}
