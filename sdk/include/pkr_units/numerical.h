...
constexpr auto stable_multiply(const T1& lhs, const T2& rhs) noexcept {
    ...
    return details::unit_t<value_type, combined_ratio, combined_dim>{
        lhs.value() * rhs.value()};
}
...
constexpr auto stable_divide(const T1& lhs, const T2& rhs) {
    ...
    if ((rhs.value() < static_cast<value_type>(0) ? -rhs.value() : rhs.value()) ==
...
constexpr auto log(const T& x) {
    ...
    return details::unit_t<typename details::is_pkr_unit<T>::value_type,
                          std::ratio<1,1>,
                          dimension_t{}>{std::log(x.value())};
}
...
StateUnit runge_kutta_step(RHS f, TimeUnit x, StateUnit y, TimeUnit h) {
    ...
    auto k2 = f(stable_add(x, half_h),
                stable_add(y, stable_multiply(half_h, k1)));
    auto k3 = f(stable_add(x, half_h),
                stable_add(y, stable_multiply(half_h, k2)));
    auto k4 = f(stable_add(x, h), stable_add(y, stable_multiply(h, k3)));

    // y + h/6*(k1 + 2*k2 + 2*k3 + k4)
    auto two = 2.0;
    auto six = 6.0;
    auto sum = stable_add(stable_add(k1, stable_multiply(two, k2)),
                         stable_add(stable_multiply(two, k3), k4));
